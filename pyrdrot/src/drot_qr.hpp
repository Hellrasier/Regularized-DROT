...
T* quadratic_regularizer_drot_wrapper(const T *_c, // cost
    ...
    // === WRITE RESULT TO DEVICE BUFFER (no host bounce) ===
    if (x_out_device != nullptr) {
        size_t mat_bytes = static_cast<size_t>(n_rows) * static_cast<size_t>(n_cols) * sizeof(T);
        cudaMemcpyAsync(x_out_device, x, mat_bytes, cudaMemcpyDeviceToDevice, stream);
        cudaStreamSynchronize(stream);
    }

    // === FREE ===
    // free only the buffers you allocated inside this wrapper
    cudaFree(c);
    cudaFree(p);
    cudaFree(q);
    cudaFree(a);
    ...
    // IMPORTANT: you previously did NOT free x when returning it to host.
    // Now you ALWAYS free x here, because ownership is with the caller's x_out_device.
    cudaFree(x);
}
